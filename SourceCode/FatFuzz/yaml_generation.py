...
# number of new test cases created by favour test cases
...
# number of new test cases created by test cases
...
# generate random number to control mutation strategy of string data type 
...
# generate random number to control how many bits will be mutated in number mutation
...
# generate random number to control mutation strategy of time data type 
...
# one of the int data type mutation strategy
def modify_bits(num):
...
# one of the int float type mutation strategy
...
# copy new test cases files to itest framework execution path
...
# scan all test cases files in directory
...
# create data.yaml file and calculate the number of new test cases
def create_data_block(data_yaml_files, data_seed_directory_path, target_path, seed_array, Max_coverage):
    ...
    for source_file in data_yaml_files:
        ...
        with open(source_file, 'r') as s_file:
            test_case_blocks = s_file.read().split('- !!com.ipay.itest.common.model.TestData') # splitting data.yaml files with regular expressions to data block

        with open(target_file_path, 'w') as t_file: # creating new data block
...
# create logicata.yaml file basing on new_seed_count from create_data_block method
def create_logic_block(logic_yaml_files, target_path, seed_array, pre_test_case_counter, Max_coverage):
    ...
    for source_file in logic_yaml_files:
        ...
        with open(source_file, 'r') as s_file:
            test_case_blocks = s_file.read().split('- !!com.ipay.itest.common.model.TestLogic') # splitting logic.yaml files with regular expressions to logic block

        with open(target_file_path, 'w') as t_file:  # creating new logic block
            for block in test_case_blocks[1:]:
                for line in block.split('\n'):
                    if "case_" in line:
...
# before create the request.yaml and mock.yaml, we need to transfer block format to dict format
def parse_block_to_dict(block):
    block_dict = OrderedDict() # because in Ant Group the version of python is old, we OrderedDict type is necessary
    for line in block.split('\n'):
        if ':' in line:
...
# create request.yaml and mock.yaml
def create_request_mock_block(request_yaml_files, request_seed_directory_path, target_path, mock_files, target_mock_directory, seed_array,
    ...
    for request_file in request_yaml_files:
        ...
        with open(request_file, 'r') as s_file:
            content = s_file.read()
            request_blocks = re.findall(r'(case_\d+:.*?(?=\ncase_\d+:|\Z))', content, re.DOTALL | re.MULTILINE) # splitting request.yaml and mock.yaml files with regular expressions to block
            ...
            if match:
                ...
                if block_index in seed_indexes:
                    if len(request_dict) > 0:
                        ...
                        elif len(crossover_seed_pool) >= crossover_size: # try to put test case into crossover pool
                        ...
                        request_file_name_lab = '#' + request_file_name + '#_' + str(counter) # add '#' at the begin and end of test cases parameter to distinguish the block from different test cases files
    ...
    for mock_file in mock_files:
        ...
        with open(mock_file, 'r') as s_file:
            content = s_file.read()
            mock_blocks = re.findall(r'(case_\d+:.*?(?=\ncase_\d+:|\Z))', content, re.DOTALL | re.MULTILINE) # same logic as request.yaml
        seed_indexes = [row[0] for row in seed_array]
        counter = 1
...
def dict_Mutation(standard_dic_kv):# mutation strategy
    ...
    if datatype_value == 'int' or datatype_value == 'long': # int and long mutation strategy
    ...
    elif datatype_value == 'enum': # enum mutation strategy
    ...
    elif datatype_value == 'mixed': # mixed date types mutation strategy
    ...
    elif datatype_value == 'map': # map mutation strategy
    ...
    elif datatype_value == 'list': # list mutation strategy which contain all type of mutation strategy
...
def mutate_string(s): # 4 kinds of string mutation strategy
...
def get_char_pool(mutation_type): # the scope of char mutation
...
def add_string(s, char_pool=None, operation_length=0): # details of add string mutation strategy
...
def trim_string(s, operation_length, offset): # details of trim string mutation strategy
...
def modify_string(s, char_pool, operation_length, offset): # details of modify string mutation strategy
    random_chars = ''.join(secure_random10.choice(char_pool) for _ in range(operation_length))
    return s[:offset] + random_chars + s[offset + operation_length:]


def generate_random_datetime_or_null(): # create a new datetime to mutate datetime datatype
...
# create request.yaml and mock.yaml by crossover and the number of test cases generated by the crossover process is calculated
...
# create logic.yaml by crossover
...
# Perform the operations before the iteration to turn one test case into multiple
...
# focus on the test cases which increase the coverage 
...
# focus on the test cases which increase the coverage, and mutate them to new test cases
...
# control the whole process include launch iTest framework, create new test cases, mutation, crossover
